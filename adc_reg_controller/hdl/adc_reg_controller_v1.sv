...
// Instantiation of Axi Bus Interface S00_AXI
	adc_reg_controller_v1_S00_AXI # ( 
		.C_S_AXI_DATA_WIDTH(C_S00_AXI_DATA_WIDTH),
		.C_S_AXI_ADDR_WIDTH(C_S00_AXI_ADDR_WIDTH)
	) adc_reg_controller_v1_S00_AXI_inst (
	...
	wire [31:0] R_clock_ctrl;
	reg [15:0] dis_count;
	wire clk_sampling;
	
	// assign clock_ctrl = R_clock_ctrl;
	sampling_time sampling_time(.scale_in(R_clock_ctrl), .time_sampling(time_sampling));
	always @(posedge s00_axi_aclk, negedge s00_axi_aresetn) begin
		if(~s00_axi_aresetn)
			dis_count <= 0;
		else begin
			dis_count <= dis_count + 1;
			if(dis_count >= time_sampling)
				dis_count <= 0;
		end
	end

	always @(posedge s00_axi_aclk) begin
		if(dis_count <= time_sampling>>1)
			controlled_clk <= 1'b1;
		else 	
			controlled_clk <= 1'b0;
	end
	// assign clk_sampling = (dis_count <= time_sampling>>1);
	// assign controlled_clk = clk_sampling;
	// User logic ends
